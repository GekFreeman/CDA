...
class MFSAN(nn.Module):
    ...
    def forward(self, data_src1, label_src1 = 0,data_src2=0, label_src2 = 0, data_tgt1 = 0, data_tgt2 = 0, step = 1,alpha=1,label_tgt1 = 0,label_tgt2= 0,target_data1=0 ,target_data2=0):
        mmd_loss = 0
...
#             data_src2_pool = torch.sqrt(data_src2_pool)0
            ...
            if step==2:
                ...
                data_src1_pool = self.avgpool(data_src1)
                pred_src1 = data_src1_pool.view(data_src1_pool.size(0), -1)                 
              #  mmd_loss += mmd.mmd(data_src1, data_tgt_son1)
                ...
                data_src2_pool = self.avgpool(data_src2)
                pred_src2 = data_src2_pool.view(data_src2_pool.size(0), -1)
                
                ########target from heapq for gate#################
                data_tgt1  = self.sharedNet(data_tgt1)
                ...
                ########target from all for mmd#################
                target_data1  = self.sharedNet(target_data1)
                target_data1 = self.sonnet1(target_data1)
                target_data1 = self.avgpool(target_data1)
                target_data1 = target_data1.view(target_data1.size(0), -1)####for mmd
                
                target_data2  = self.sharedNet(target_data2)
                target_data2 = self.sonnet1(target_data2)             
                target_data2 = self.avgpool(target_data2)
                target_data2 = target_data2.view(target_data2.size(0), -1)####for mmd
                
                mmd_loss = mmd.mmd(pred_src1, target_data1)+ mmd.mmd(pred_src2, target_data2)
                mmd_loss /= (1 + alpha)
                
                
                
                pred_target1 = self.cls_fc_son1(target_data1)
               # pred_tgt1=nn.functional.softmax(pred_tgt1, dim=1)
               # pred_target1 = pred_tgt1.data.max(1)[1]
                
                pred_target2 = self.cls_fc_son1(target_data2)
              #  pred_tgt2=nn.functional.softmax(pred_tgt2, dim=1)
               # pred_target2 = pred_tgt2.data.max(1)[1]
           ...
           #######sigmoid-gate#########
               ...
               # print(self.gate.shape)
                ...
                ##########################
                    ...
                    pred_src2=data_src2_pool
                    pred_src2 = pred_src2.view(pred_src2.size(0), -1)
                  #  data_tgt_son1 = self.avgpool(data_tgt1)
                   # data_tgt_son1 = data_tgt_son1.view(data_tgt_son1.size(0), -1)
                    # mmd.mmd(pred_src1, data_tgt_son1)
