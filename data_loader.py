...
def load_training_target(root_path, dir, batch_size, kwargs):
    ...
    cls_num = len(data.classes)
    
    data_cls=list(i for i in range(cls_num))
    data_clsset=list(data.classes[i] for i in data_cls)
    
    train_loader1 = torch.utils.data.DataLoader(myImageFloder(root=root_path + dir, label=data_clsset,index=data_cls, transform=transform),batch_size=batch_size, shuffle=True,drop_last=True, **kwargs)
    train_loader2 = torch.utils.data.DataLoader(myImageFloder(root=root_path + dir, label=data_clsset,index=data_cls, transform=transform),batch_size=batch_size, shuffle=True,drop_last=True, **kwargs)
   # train_loader = torch.utils.data.DataLoader(data, batch_size=batch_size, shuffle=True, drop_last=True, **kwargs)
    return train_loader1,train_loader2 
def load_sameclass_target(dic,source_data1, source_label1, source_data2,source_label2,batch_size):
   ...
   # print(source_label1,source_label2)
    train_loader1,loader1label,loader1path =myImageTargetFloder(dic=dic, index=data_cls1, transform=transform)
    train_loader2,loader2label,loader2path  =myImageTargetFloder(dic=dic, index=data_cls2, transform=transform)
    #torch.utils.data.DataLoader(myImageTargetFloder(dic=dic, index=data_cls1, transform=transform),batch_size=batch_size, shuffle=True)
   # train_loader2 =torch.utils.data.DataLoader(myImageTargetFloder(dic=dic, index=data_cls2, transform=transform),batch_size=batch_size, shuffle=True)
   # train_loader = torch.utils.data.DataLoader(data, batch_size=batch_size, shuffle=True, drop_last=True, **kwargs)
    #import pdb
   # pdb.set_trace()
    return train_loader1,loader1label,loader1path,train_loader2,loader2label,loader2path
